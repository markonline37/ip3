...
<p>The purpose of this page is to provide a tutorial to the earthquake visualization which can be found <a href="earthquake.php">here</a>.<br>
...
<p>Earthquakes originate from a sudden release of energy on or in the earth. The causes can vary and the effects can be devastating, <br> 
thus it is important to document these and provide the public with knowledge about them. </p>
<p>There is usually a specific point as the origin, and it is usually the spot where tectonic collide with each other, known as a fault. <br>
This point would be recorded as the origin, so the location would be entered as the data for longitude and latitude. </p>
<p>The richter scale measures the size of the earthquake. The higher an earthquake is on the richter scale, the more destruction it is likely to cause,<br>
so a sorting functionality would be useful to filter out those that just cause slight tremors. <br>
For more information on earthquakes this website can be visited:</p>
 <a href="https://www.explainthatstuff.com/earthquakes.html">https://www.explainthatstuff.com/earthquakes.html</a>

<h2>Resources</h2>

<p>The google maps api is one of the more significant elements in the development of this page. This is retrieved, using a key, from:<br>
https://maps.googleapis.com/maps/api/js?key=[key]&callback=initMap.<br>
This allows the website to display a map which the user can navigate and zoom in and out of. This key is protected by making it ip-restricted, such that the <br> 
server would be the only client able to use the key.  </p>

<p> The earthquake feeds are received from <a href="https://earthquake.usgs.gov/earthquakes/feed/v1.0/">https://earthquake.usgs.gov/earthquakes/feed/v1.0/</a> . This website provides GeoJSON data to earthquakes in specific timeframes. <br>GeoJSON is essentially just a format used to represent geographical data. More about this data format can be read  <a href="geojsontutorial.php"> here </a>. </p>

<p>One of the requirements for the website is that the data shown on  the map is dynamically clustered. This is achieved by using a google-provided set of Javascript code: <br>
<a href="https://developers.google.com/maps/documentation/javascript/examples/markerclusterer/markerclusterer.js">https://developers.google.com/maps/documentation/javascript/examples/markerclusterer/markerclusterer.js</a><br>
This interacts with the google maps api previously mentioned to provide a clustering functionality to markers on the map. <br>
What this will do in the end product is group together earthquakes and indicate the amount of earthquakes <br>
in the cluster. As the user zooms in, the cluster disappears and the individual earthquakes show.</p>

<p>Another part of the website is the info-windows, which pop up as the user clicks on the earthquake markers. These open<br>
a new tab and display information about the location. The locational information for this part is sourced from <br>
https://api.opencagedata.com/geocode/v1/json?q=[latitude]%2C%20[longitude]&key=[key]&language=en&pretty=1.<br>
Naturally, the user would need a key and a latitude and longitude to use this url. 
</p>
<p>This part of the website also displays the flag of the relevant country, which is gotten from https://www.countryflags.io/[countryCode]/flat/64.png. <br>The country code from opencagedata is used to display the flag. </p>
<p>
	A key part of this website is the retrieval of JSON data. How is this done? In this case, using JQuery, which is essentialy just a javascript library. JQuery also <br>
	includes functionality for ajax requests, which are used to update the page with data as it is running. This website uses ajax to import data by importing the jquery library:<br><a href="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js">https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js</a>
</p>
