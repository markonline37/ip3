...
<p>
	Earthquakes originate from a sudden release of energy on or in the earth. The causes can vary and the effects can be devastating, <br> 
	thus it is important to document these and provide the public with knowledge about them. 
</p>
<p>
	There is usually a specific point as the origin, and it is usually the spot where tectonic collide with each other, known as a fault. <br>
	This point would be recorded as the origin, so the location would be entered as the data for longitude and latitude. 
</p>
<p>
	The richter scale measures the size of the earthquake. The higher an earthquake is on the richter scale, the more destruction it is likely to cause,<br>
	so a sorting functionality would be useful to filter out those that just cause slight tremors. <br>
	For more information on earthquakes this website can be visited:
</p>
...
<p>
	...
	server would be the only client able to use the key.  
</p>
...
<p> 
	...
	GeoJSON is essentially just a format used to represent geographical data. More about this data format can be read  <a href="geojsontutorial.php"> here </a>. 
</p>
...
<p>
	...
	in the cluster. As the user zooms in, the cluster disappears and the individual earthquakes show.
</p>

<p>
	Another part of the website is the info-windows, which pop up as the user clicks on the earthquake markers. These open<br>
...
<p>
	This part of the website also displays the flag of the relevant country, which is gotten from https://www.countryflags.io/[countryCode]/flat/64.png. <br>The country code from opencagedata is used to display the flag. 
</p>
<p>
	...
	includes functionality for ajax requests, which are used to update the page with data as it is running. This website uses ajax to import data by importing the jquery library:<br>
...
<p>
	...
	This will be looped through to create the buttons that alter the data feed.<br>
	<p>
		Once the page is loaded javascript calls this function:
		<pre>
		...
		The "resizer" function adjusts the map size according to the window's size.
	</p>

	<p>
		The bulk of the js happens once the google maps api code is loaded, which triggers the initMap() function. <br>
	...
	<p>
		...
		the position of the markers on the map are set with longitude and latitude. <br>
		<pre>
